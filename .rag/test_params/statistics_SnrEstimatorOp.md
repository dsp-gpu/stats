...
schema_version: 1
repo: stats
class_fqn: statistics::SnrEstimatorOp
file: E:/DSP-GPU/stats/include/stats/operations/snr_estimator_op.hpp
line: 52
brief: "Оценка SNR через CFAR-пайплайну с FFT и медианой"
methods_total: 2
methods_with_doxygen: 2
ai_generated: true
human_verified: false
parser_version: 2
synonyms_ru: ['SnrEstimator', 'SNR-estimator', 'SNR оценщик', 'SNR estimator', 'CFAR пайплайна', 'SNR pipeline']
synonyms_en: ['SnrEstimator', 'SNR-estimator', 'SNR estimator', 'CFAR pipeline', 'SNR processor', 'SNR calculator']
tags: ['SNR', 'GPU', 'CFAR', 'FFT', 'медиана', 'статистика', 'ROCm', 'HIP']
---

# `statistics::SnrEstimatorOp` — карточка класса
...
**ЧТО**: Оценка SNR через CFAR-пайплайну с FFT и медианой

**ЗАЧЕМ**: Решает задачу оценки отношения сигнал/шум в условиях отражений и шума с использованием GPU-оптимизированных алгоритмов

**КАК**: Lazy init FFT-процессора, shared_buf для буферов, этапы gather → FFT → peak CFAR → медиана, асинхронные копирования через HIP

**Пример**:
```cpp
#include "stats/operations/snr_estimator_op.hpp"
using namespace stats::operations;

int main() {
  SnrEstimatorOp op;
  op.SetupFft(backend);
  op.Initialize(ctx);
  SnrEstimationConfig config;
  SnrEstimationResult result;
  op.Execute(gpu_input, 4, 1024, config, result);
  return 0;
}
```
